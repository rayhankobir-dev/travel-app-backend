...
export const getServices = asyncHandler(async (req, res) => {
  try {
    ...
    return res.status(200).json(new ApiResponse(200, "Success", {services}))
  } catch(error) {
    throw error;
  }
})
...
export const editService = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const updatedService = await TourService.findByIdAndUpdate(serviceId, data, {new: true});
...
export const deleteService = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    await TourService.findByIdAndDelete(service._id);
