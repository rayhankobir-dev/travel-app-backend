...
export const modifyOrder = asyncHandler(async (req, res) => {
  const { orderId, totalPerson } = req.body;
  try {
    const order = await Order.findById(orderId);
    const difference = totalPerson - order.totalPerson;

    if (difference < order.totalPerson) {
      const refundAmount = -difference * order.perPersonCost;
      await initiateRefund(order.transactionId, refundAmount);
    } else if (difference > order.totalPerson) {
      const extraCost = difference * order.perPersonCost;
      const paymentLink = await generatePaymentLink(extraCost);
      res.send(paymentLink);
    }

    order.totalPerson = totalPerson;
    await order.save();
  } catch (error) {
    throw error;
  }
